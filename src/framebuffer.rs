...
impl Framebuffer {
    ...
    // Guarda la imagen en disco como PNG o JPG según la extensión
    match std::path::Path::new(filename).extension().and_then(|e| e.to_str()) {
        Some("png") => self.image.export_image(filename),
        Some("jpg") | Some("jpeg") => self.image.export_image(filename),
        _ => println!(" Formato no soportado. Usa .png o .jpg"),
    }
